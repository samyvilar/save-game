...
class nonPrivateUpload(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium("localhost", 4444, "*chrome", "http://127.0.0.1:8000/")
        self.selenium.start()
    
    def test_non_private_upload(self):
        sel = self.selenium
        sel.open("/signOut/")
        ...
        sel.type("id_title", "Non-Private Upload Test")
        sel.type("id_description", "Non-Private Upload Test\n== Description ==")
        ...
        sel.click("logo_title")
        sel.wait_for_page_to_load("30000")
        try: self.failUnless(sel.is_text_present("Non-Private Upload Test"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.failUnless(sel.is_text_present("WTF: Work Time Fun"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        sel.click("link=Townes Wang")
        sel.wait_for_page_to_load("30000")
        try: self.failUnless(sel.is_text_present("Non-Private Upload Test"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.failUnless(sel.is_text_present("WTF: Work Time Fun"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.failUnless(sel.is_text_present("Non-Private Upload Test == Description =="))
        except AssertionError, e: self.verificationErrors.append(str(e))
